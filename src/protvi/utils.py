...
def reshape_anndata_like(adata, adata_like, sanity_check=True, verbose=True):
    ...
    if verbose:
        print(f"transferring {len(row_overlap)} cells and {len(col_overlap)} proteins during reshape.")
...
def prepare_anndata_for_R(adata):
    ...
    adata.obs = pd.DataFrame.from_dict(
        {
            column: adata.obs[column].astype(np.int32) if adata.obs[column].dtype == np.int64 else adata.obs[column]
