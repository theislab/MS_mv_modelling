...
def plot_loss(history, epoch_start=0, validation_smooth_window=None, pad=3):
    ...
    if validation_smooth_window is not None:
        ...
        _smooth(history, "reconstruction_loss_validation", window=validation_smooth_window)
...
def scatter_protein_detection_proportion_and_intensity2(x, title=None, ax=None):
    ...
    ax.scatter(x_obs_protein, p_protein, color="b", alpha=0.15, s=3)
    ax.set_title(title)
...
def plot_protein_detection_proportion_panel(x, p_est, x_est=None, color="blue", title="PROTVI"):
    ...
    _scatter_compare_protein_detection_proportion(p_protein, p_est_protein, color=color, ax=axes[1])
    _hist_compare_protein_detection_proportion_difference(p_protein, p_est_protein, color=color, ax=axes[2])
...
def _scatter_compare_protein_detection_proportion(p_protein, p_est_protein, color="blue", ax=None):
    if ax is None:
        fig, ax = plt.subplots(figsize=(6, 6))
...
def _hist_compare_protein_detection_proportion_difference(p_protein, p_est_protein, color="blue", ax=None):
    if ax is None:
        fig, ax = plt.subplots(figsize=(6, 6))
...
def _scatter_compare_protein_detection_proportion_difference(
    ...
    diff = p_est_protein - p_protein
    ax.scatter(x_protein, diff, color=color, edgecolor="black", linewidth=0, s=6, alpha=0.5)
    ax.set_xlabel("Avg. observed log-intensity")
...
def plot_protein_intensity_panel(x, x_est, title="PROTVI"):
    ...
    x_est_obs_protein_shared = np.nanmean(x_est_obs[:, protein_with_missing_intensities_mask], axis=0)
    x_est_miss_protein_shared = np.nanmean(x_est_miss[:, protein_with_missing_intensities_mask], axis=0)

    ax = axes[2]
    diff = x_est_obs_protein_shared - x_est_miss_protein_shared
    ax.hist(diff, bins=60, color="red", edgecolor="black", linewidth=1.2)
    ax.set_ylabel("Number of proteins")
    ax.set_xlabel("Avg. pred. observed log-intensity - avg. pred. missing log-intensity")
    ax.grid(True)
    ax.set_axisbelow(True)
...
def plot_model_intensity_comparison(
    ...
    ax.set_xlabel("Predicted missing protein log-intensity")
    ax.set_ylabel("Predicted observed protein log-intensity")
    ax.set_title("Predicted missing intensity vs. predicted observed intensity - for each protein")
    ax.legend(markerscale=2)
    ax.grid(True)
    ax.set_axisbelow(True)
...
def plot_protein_detection_proportion_panel_protDP(x, protdp_result, color="blue", title="protDP"):
...
def plot_mnar_mcar_ratio(adata, m_mnar, m_mcar, normalize=False, ax=None):
    ...
    ax.bar(
        edges_mnar[:-1], bars_mnar + bars_mcar, width=np.diff(edges_mnar), align="edge", edgecolor="black", label="MNAR"
    )
